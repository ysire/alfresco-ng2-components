...
Request.prototype.type = jest.genMockFunction().mockReturnThis();
Request.prototype.timeout = jest.genMockFunction().mockReturnThis();
...
Request.prototype.end = jest.genMockFunction().mockImplementation(function(callback) {
    ...
    callback.respondWith = function(response){
    ...
    jasmine.Ajax.requests.track(callback)
...
module.exports = function(method, url) {
    ...
    if ('function' == typeof url) {
        return new superagent.Request('GET', method).end(url);
    }

    // url first
    if (1 == arguments.length) {
        return new superagent.Request('GET', method);
    }

    return new superagent.Request(method, url);
}
